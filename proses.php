...
if(isset($_POST["register"])){
   ...
   if($_POST["email"] =="" | $_POST["username"] == "" | $_POST["token"] == "" | $_POST["password"] == ""){
  ...
  //jika email si pengguna register tidak ada dlm table login
   ...
   $save = mysqli_query($conn,"INSERT INTO login(username,email,password,token) VALUES('$username', '$email', '$password', '$token')");
...
///////////////////////////////////////
  if(isset($_POST["tambah_pengguna"])){
   if($_POST["email"] =="" | $_POST["username"] == "" | $_POST["password"] == "" | $_POST["token"] =="" | $_POST["level"] == ""){
      echo'<script>
             alert("Jangan ada yang kosong !");
  ...
  //save
   $save = mysqli_query($conn,"INSERT INTO login(username,email,password,level,token) VALUES('$username', '$email', '$password', '$level', '$token')");
   if ($save === true) {
        echo'<script>
                alert("Pengguna Baru Berhasil Ditambahkan...");
                window.location="'.$base_url.'/admin/daftar-pengguna.php";
             </script>';
   }else{
         echo'<script>
  ...
  if(isset($_SESSION["username"])){
    ...
    $pengguna = mysqli_fetch_assoc($data);
      ...
      if($_SESSION["level"] == "admin"){
        ...
        $enters = mysqli_query($conn,"SELECT * FROM login");

        $tokens = mysqli_query($conn,"SELECT * FROM login WHERE token='token'");
